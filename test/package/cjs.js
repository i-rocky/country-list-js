...
ok('13 members', Object.keys(now).length === 13);
assert.deepStrictEqual(Object.keys(now).sort(), Object.keys(old).sort(), 'member names');
checks++;
...
for (const iso2 of Object.keys(old.all)) {
    const e = old.findByIso2(iso2), a = now.findByIso2(iso2);
...
// 4. every deep import path the 3.1.8 tarball exposed
const fs = require('fs'), path = require('path');
const oldData = fs.readdirSync(path.dirname(require.resolve('old/package.json')) + '/data');
for (const f of oldData) {
    const p = 'country-list-js/data/' + f;
    ok('deep import ' + p, require.resolve(p) && typeof require(p) === 'object');
}
...
const seen = []; for (const k in [1,2]) seen.push(k);
ok('no Array.prototype leak', !seen.includes('unpack') && !seen.includes('unique'));
ok('destructuring works', (({names}) => names().length)(now) === 250);
now.findByName('Denmark').name = 'MUTATED';
ok('mutation does not poison the cache', now.findByName('Denmark').name === 'Denmark');
...
ok('longestMatch', now.findByPhoneNbr('+12465551212', {longestMatch:true}).name === 'Barbados');
